...
public class MyRectTransformExtended : MonoBehaviour {
...
#if UNITY_EDITOR
[CustomEditor(typeof(MyRectTransformExtended))]
public class MyRectTransformExtendedInspector : Editor
{
    [SerializeField]
    private bool HelpFolded;
    private MyRectTransformExtended Script;

    public override void OnInspectorGUI()
    {
        Script = (MyRectTransformExtended)target;

        Script.GetReferences();

        Undo.RecordObject((RectTransform)Script.transform, "Changed MyRectTransformExtended");
        ...
        {
            Script.PositionIgnoringAnchorsAndPivot = PositionControl(new GUIContent("parent left, bottom - to pivot", "To control the position using a value relative to the parent, ignoring anchors position. Without this the only option is to control position with a value relative to the anchors using RectTransform.AnchoredPosition. \n \nUsefull to ignore anchors in one or both axis."), Script.PositionIgnoringAnchorsAndPivot);
